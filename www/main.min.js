...
!function(){"use strict";function t(t,a,r){var e=t.hash().split("=")[1];r.saveToken(e),a.go("auth")}function a(a,r,e){a.interceptors.push("authInterceptor"),r.state("auth",{url:"/auth",templateUrl:"./fsr/auth/index.html",controller:"AuthController as auth"}),e.when("/auth/callback/",["$location","$state","authFactory",t])}function r(t,a,r){t.$on("$stateChangeStart",function(t,e){return"auth"===e.name?!1:void(r.isLoggedIn()||(t.preventDefault(),a.go("auth")))})}angular.module("auth",["ui.router","angular-storage"]).config(["$httpProvider","$stateProvider","$urlRouterProvider",a]).run(["$rootScope","$state","authFactory",r])}();
