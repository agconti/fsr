!function(){"use strict";function o(o,i){o.ready(function(){window.cordova&&window.cordova.plugins&&window.cordova.plugins.Keyboard&&(cordova.plugins.Keyboard.hideKeyboardAccessoryBar(!0),cordova.plugins.Keyboard.disableScroll(!0)),window.StatusBar&&StatusBar.styleLightContent(),i.go("list")})}function i(o,i,t,r){o.html5Mode(!0),t.otherwise(function(o){var i=o.get("$state");i.go("list")})}angular.module("fsr",["ionic","config","auth","lists","venues"]).run(["$ionicPlatform","$state",o]).config(["$locationProvider","$stateProvider","$urlRouterProvider","$injector",i])}();
!function(){"use strict";function t(t,a){a.state("auth",{url:"/auth",templateUrl:"./fsr/auth/index.html",controller:"AuthController as auth"})}function a(t,a,r){t.$on("$stateChangeStart",function(t,o){return"auth"===o.name?!1:r.isLoggedIn()?void 0:(t.preventDefault(),a.go("auth"))})}angular.module("auth",["ui.router","angular-storage","ngCordovaOauth"]).config(["$httpProvider","$stateProvider",t]).run(["$rootScope","$state","authFactory",a])}();
